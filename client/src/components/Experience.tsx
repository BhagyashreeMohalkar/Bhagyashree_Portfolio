    ...
    <section id="experience" className="py-16 md:py-20">
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          ...
          <p className="text-gray-400 mt-3 text-sm md:text-base">My journey</p>
        </motion.div>
        ...
        <div className="max-w-full md:max-w-5xl mx-auto relative px-2 md:px-0">
          ...
          <div className="space-y-8 md:space-y-12">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline Dot */}
                <div className="absolute left-0 md:left-[29px] top-0 w-2.5 h-2.5 rounded-full bg-primary shadow-[0_0_10px_rgba(139,92,246,0.5)]" />
                ...
                <div className="relative group">
                  ...
                  <div
                    ...
                    className="absolute inset-0 rounded-2xl -z-10 pointer-events-none
                               ...
                               opacity-0 blur-[30px] md:blur-[40px] transform scale-95
                               transition-all duration-400 group-hover:opacity-60 group-hover:blur-[80px] group-hover:scale-100"
                  />

                  {/* Card */}
                  <div className="bg-card border border-white/5 rounded-2xl p-5 md:p-8
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between mb-3 gap-2">
                      <div className="pr-0 md:pr-6 lg:pr-8">
                        ...
                        <p className="text-base md:text-lg font-medium text-blue-400">{exp.company}</p>
                      </div>

                      {/* Inline pill (mobile) */}
                      <span className="md:hidden text-gray-500 font-mono text-xs bg-white/5 px-2 py-1 rounded-full self-start mt-2">
                        {exp.period}
                      </span>

                      {/* Absolute pill (desktop) */}
                      <span
                        className="hidden md:inline-block absolute right-4 md:right-6 top-4 text-gray-500 font-mono text-sm bg-white/5 px-3 py-1 rounded-full"
                        aria-hidden
                      >
                        {exp.period}
                      </span>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.tech.map((tech) => (
                        <span 
                          key={tech} 
                          className="px-3 py-1 rounded-full text-xs md:text-xs font-medium bg-white/5 border border-white/5 text-gray-400 hover:text-white hover:border-white/20 transition-colors cursor-default"
                        >
                          {tech}
                        </span>
                      ))}
